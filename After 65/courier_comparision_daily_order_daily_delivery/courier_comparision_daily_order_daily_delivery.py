...
where  r.company = %s  and r.{date_field} between %s and %s
			...
			"""
			.format(date_field=self.date_field, value_field=value_field ,value_field2=value_field2),
	...
	def get_teams(self):
		...
		self.group_entries = frappe.db.sql("""select name,parent from
(
